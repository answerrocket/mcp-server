...
install_mcp_servers() {
    ...
    while IFS='|' read -r copilot_id copilot_name; do
        if [[ -n "$copilot_id" ]]; then
            ...
            if uv run mcp install mcp_server/main.py -n "$server_name" -v "AR_URL=$ar_url" -v "AR_TOKEN=$ar_token" -v "COPILOT_ID=$copilot_id" --with "answerrocket-client" --with "fastmcp" ; then
